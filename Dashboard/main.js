...
const serial = async (
    ...
    arduino.pipe(new serialport.ReadlineParser({ delimiter: '\r\n' })).on('data', async (data) => {
        ...
        const name = parseFloat(valores[0]);
        const email = parseFloat(valores[1]);
        const phone = parseFloat(valores[2]);
        const password = parseFloat(valores[3]);
        const cpf = parseFloat(valores[4]);
